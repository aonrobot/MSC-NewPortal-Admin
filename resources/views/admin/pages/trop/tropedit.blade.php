...
<form action="<?=asset('/admin/trop/editupdate')?>" method="get">
 ...
 <h1><i class="fa fa-edit" aria-hidden="true"></i> Trop Setting</h1>
	  ...
	  @if(!$trop_tid)
		 ...
		 Subtitle <input value="Portal" name="subtitle" style="width: 14%" readonly >
	  @else
		 ...
		 Subtitle <input value="<?php echo $detail1[0]->trop_subtitle; ?>" name="subtitle" style="width: 14%" >
	  @endif
		 Slide Head
         <select name="Slide1">
		  <option value="">-------------</option>
...
foreach ($slide as $slide1) {
	...
	?>
            <option value="<?php echo $slide1->slide_id ?>"<?php if ($slide1->slide_id == $setting[0]->set_value) {
		echo "selected";
	}
	?> ><?php echo $slide1->slide_name ?></option>
            <?php }?>
         </select>
		@if(!$trop_tid)
		 Slide Home
...
foreach ($slide as $slide1) {
	error_reporting(E_ALL ^ E_NOTICE);
	?>
            <option value="<?php echo $slide1->slide_id ?>"<?php if ($slide1->slide_id == $setting[1]->set_value) {
		echo "selected";
	}
	?> ><?php echo $slide1->slide_name ?></option>
            <?php }?>
...
foreach ($menuportal as $menuportal1) {
	error_reporting(E_ALL ^ E_NOTICE);
	?>
            <option value="<?php echo $menuportal1->mid ?>"<?php if ($menuportal1->mid == $setting[2]->set_value) {
		echo "selected";
	}
	?>><?php echo $menuportal1->menu_name ?></option>
            <?php }?>
         </select>
		 @else
		 Menu Setting
...
foreach ($menu as $menu1) {
	error_reporting(E_ALL ^ E_NOTICE);
	?>
            <option value="<?php echo $menu1->mid ?>"<?php if ($menu1->mid == $setting[1]->set_value) {
		echo "selected";
	}
	?>><?php echo $menu1->menu_name ?></option>
            <?php }?>
         </select>
   ...
   <table id="trop_admin" class="table table-bordered">
	 ...
	 <tbody>
	    ...
	    <tr>
         ...
         <td><center>{{$em_info->NickName}}</center></td>
         <td><center>
		  Admin Trop
		</center></td>
