...
class UploadController extends Controller {
	...
	public function index($component, $trop, $post) {

		$path = '/trop/' . $trop . '/post/' . $post;

		$options['upload_dir'] = Config::get('newportal.upload_dir') . $path . '/' . $component . '/';
		//Uploads custom folder
		$options['upload_url'] = Config::get('newportal.upload_url') . $path . '/' . $component . '/';

		//Create handler with options
		$upload_handler = new UploadHandler($options);
	}

	public function thumbnail($object, $tropId, $objectId) {

		$path = '/trop/' . $tropId . '/' . $object . '/' . $objectId;

		$options['upload_dir'] = Config::get('newportal.upload_dir') . $path . '/thumbnail/';
		//Uploads custom folder
		$options['upload_url'] = Config::get('newportal.upload_url') . $path . '/thumbnail/';
	...
	public function slide($slideId) {
		...
		$options['upload_dir'] = Config::get('newportal.upload_dir') . $path . '/image/';
		//Uploads custom folder
		$options['upload_url'] = Config::get('newportal.upload_url') . $path . '/image/';
	...
	/*public function update_db(Request $request) {
		...
		if ($component == 'video' || $component == 'image') {
			...
			$url = 'http://mis_test.metrosystems.co.th/newportal/uploads' . $path . '/' . $component . '/' . $filename;
			...
			foreach ($filenames as $filename) {
				array_push($urls, 'http://mis_test.metrosystems.co.th/newportal/uploads' . $path . '/' . $component . '/' . $filename);
			}
...
class UploadHandler {
	...
	public function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		$this->options = array(
			...
			'upload_dir' => Config::get('newportal.upload_dir'), //dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/',
			'upload_url' => Config::get('newportal.upload_url') . '/', //$this->get_full_url().'/files/',
...
*/
	...
	protected function get_file_name($file_path, $name, $size, $type, $error,
		...
		$name = $date_now . '_' . preg_replace('/[^A-Za-z0-9 !@#$%^&*().]/u', '', strip_tags($name));
